...
> 版本：v0.1（2026-08-30，J1 完成时确立）。**本文档是后续所有里程碑（J2-J5）开发 agent 必读的开发约定**，与 [PLAN.md](PLAN.md)（里程碑计划）、[数据库设计说明.md](数据库设计说明.md)（表结构）、[openapi-phase2.json](openapi-phase2.json)（API 契约基线）配套使用。若本文档与 PLAN.md 冲突，以 PLAN.md 为准并同步修订本文档。
...
3. **建 PO**（`po/`）：表名 → 类名（`manual_knowledge` → `ManualKnowledge`）；字段蛇形 → 驼峰；`TIMESTAMPTZ` → `OffsetDateTime`；`VECTOR` → `PGvector`。纯 public 字段、无 Lombok（对齐现有风格）。
...
7. **建 Service**（`service/`）：构造器注入；事务边界 @Transactional；异常走 ApiException / ValidationException（见 §7）。
